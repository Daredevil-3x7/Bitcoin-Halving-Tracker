...
export default function HalvingGame() {
  ...
  const saveToLeaderboard = useCallback(
    (name: string, score: number, levelReached: number) => {
      ...
      localStorage.setItem("taoInvadersLeaderboard", JSON.stringify(updated))
  ...
  }, [gameStarted]) // Dependency on gameStarted to re-run init on start
  ...
  const saveScore = () => {
    if (playerName.trim()) {
      ...
      localStorage.setItem("tao-halving-leaderboard", JSON.stringify(newLeaderboard))
      setShowNameInput(false)
        ...
        <div className="flex flex-col lg:flex-row gap-8 items-start justify-center">
          ...
          <div className="relative">
            <canvas
              ...
              width={700}
              height={500}
              className="border border-white/20 rounded-lg bg-[#0a0a0f]"
            />
            ...
            {!gameStarted && (
              <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 backdrop-blur-sm rounded-lg">
                ...
                <p className="text-white/70 mb-8 text-center max-w-md">
                  Defend the network! Destroy subnet nodes to earn TAO rewards. Complete levels before time runs out or
                  face slashing penalties!
                </p>
